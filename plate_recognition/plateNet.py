...
class myNet_ocr(nn.Module):
    ...
    OCR网络模型，用于车牌字符的识别。
    cfg: 网络配置参数，定义了各层的参数。
    num_classes: 输出类别数，对于车牌识别，通常是车牌上可能的字符集大小。
    export: 一个布尔值，决定网络的输出格式，便于不同的使用场景。
    """
    def __init__(self,cfg=None,num_classes=78,export=False):
        super(myNet_ocr, self).__init__()
        if cfg is None:
            cfg =[32,32,64,64,'M',128,128,'M',196,196,'M',256,256]  # 默认配置
            # cfg =[32,32,'M',64,64,'M',128,128,'M',256,256]
        self.feature = self.make_layers(cfg, True)  # 构建特征提取层
        self.export = export
        # self.classifier = nn.Linear(cfg[-1], num_classes)
        # self.loc =  nn.MaxPool2d((2, 2), (5, 1), (0, 1),ceil_mode=True)
        # self.loc =  nn.AvgPool2d((2, 2), (5, 2), (0, 1),ceil_mode=False)
        self.loc =  nn.MaxPool2d((5, 2), (1, 1),(0,1),ceil_mode=False)  # 定义一个最大池化层
        self.newCnn=nn.Conv2d(cfg[-1],num_classes,1,1)  # 定义1x1卷积层以用于分类
        # self.newBn=nn.BatchNorm2d(num_classes)

    def make_layers(self, cfg, batch_norm=False):
        """
        从配置中构建网络层。
        cfg: 层配置，包含卷积层和池化层的参数。
        batch_norm: 是否在卷积层后添加批归一化。
        """
        layers = []
        in_channels = 3  # 初始通道数为3，对应RGB图像
            ...
            if i == 0:
                ...
                if cfg[i] == 'M':
                    ...
                    if batch_norm:
                        ...
                        layers += [conv2d, nn.ReLU(inplace=True)]
                    in_channels = cfg[i]
        return nn.Sequential(*layers)  # 返回一个由所有层组成的序列模型
